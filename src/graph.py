...
ענה על השאלה על סמך ההקשר מהמסמכים להלן.

## הנחיות לתשובה מעמיקה:
- **היה יסודי ומקיף** — כסה את כל ההיבטים הרלוונטיים שמופיעים בהקשר. אל תסתפק בתשובה שטחית.
- **פרט ותן דוגמאות** — אם ההקשר מכיל נתונים, מספרים, תאריכים או דוגמאות — שלב אותם בתשובה.
- **מבנה ברור** — השתמש בכותרות (##), רשימות ממוספרות, וטבלאות כשיש השוואות או נתונים מרובים.
- **ציין מקורות** — הפנה לסימוכין [1], [2] וכו' בכל טענה מהותית.
- אם אין מספיק מידע בהקשר — ציין זאת בבירור ופרט מה חסר.
- אל תמציא מידע שלא מופיע בהקשר.
...
קיבלת את התוכן המלא של המסמך/ים להלן.

## הנחיות לתשובה מעמיקה:
- **כיסוי מלא** — עבור על כל הנקודות המרכזיות במסמך. אל תדלג על חלקים רלוונטיים.
- **מבנה היררכי** — ארגן עם כותרות ראשיות (##) ומשניות (###), רשימות, וטבלאות.
- **פרט ותן דוגמאות** — שלב נתונים, מספרים ודוגמאות קונקרטיות מהמסמך.
- **סיכום תחתון** — סיים בסיכום קצר של הנקודות העיקריות.

{{context}}"""

SYNTHESIS_SYSTEM_PROMPT = f"""{APPRAISER_PERSONA}

להלן תוצאות מחיפושים מרובים בתת-שאילתות שונות.

## הנחיות לסינתזה מעמיקה:
- **שלב את כל המקורות** — אל תתעלם ממידע מאף תת-שאילתה. כסה את כל ההיבטים.
- **זהה קשרים** — הצלב מידע בין המקורות השונים, ציין התאמות וסתירות.
- **מבנה ברור** — ארגן את התשובה לפי נושאים עם כותרות, לא לפי סדר השאילתות.
- **פרט ותן דוגמאות** — שלב נתונים ודוגמאות קונקרטיות מכל מקור.
- **ציין מקורות** — הפנה לסימוכין [1], [2] וכו' כשרלוונטי.
- אם תת-שאילתה לא החזירה תוצאות שימושיות — ציין זאת.
